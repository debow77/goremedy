...
type Company struct {
	...
	// Add any other fields that are present in the original Company class
}
...
func NewCompanyClientGroup(client *RemedyClient) *CompanyClientGroup {
...
func NewCompanyQueryClient(client *RemedyClient) *CompanyQueryClient {
...
func (cq *CompanyQueryClient) getPaginated(urlPath string, params url.Values, filters map[string]string) ([]*Company, error) {
	...
	for page := 0; page < totalPages; page++ {
		...
		resp, err := cq.client.rapidClient.Get(cq.getPath()+urlPath, params)
		if err != nil {
			return nil, err
		}
